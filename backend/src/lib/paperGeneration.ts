...
export function normalizePaper(parsed: GeneratedPaper, input: GenerationInput): GeneratedPaper {
...
/** Detect template mock phrasing for tests. */
export function isLikelyMockQuestion(text: string): boolean {
  return /Which option best explains the key idea from/i.test(text);
}
